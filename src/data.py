...
from webdataset.tariterators import base_plus_ext, url_opener, tar_file_expander, valid_sample, group_by_keys
...
def get_dataset_size(shards, combine=False):
    if not combine:
...
def text_process_cfg(text, cfg):
    p = random.random()
    if p < cfg:
        return ""
    return text

def decode_webdataset_256(src):
    for sample in src:
        try:
            if "0.png" in sample:
            ...
            sample["png"] = img_process_256(img)
...
def decode_webdataset_512(src):
    for sample in src:
        try:
            ...
            with io.BytesIO(img) as stream:
                img = Image.open(stream).convert("RGB")
            sample["png"] = img_process_512(img)
...
def filter_no_caption_or_no_image_or_notm18(sample):
    ...
    if has_caption:
        ...
        is_18_language = language in M18_LANGUAGES
...
def get_wds_dataset(args, is_train, epoch=0, floor=False):
    ...
    num_samples, num_shards = get_dataset_size(input_shards, args.use_combine)
    if not num_samples:
        if is_train:
    ...
    # TODO: use_ourdecoder need to add cfg flag!
    if args.use_ourdecoder:
        if args.use_256:
            pipeline.extend([
                wds.select(filter_no_caption_or_no_image),
                decode_webdataset_256,
        ...
        if args.use_256:
            pipeline.extend([
                    ...
                    wds.rename(image="jpg;png;jpeg;webp;0.png", text="txt;0.txt"),
                    wds.map_dict(image=img_process_256, text=lambda text: text_process_cfg(text, args.cfg)),
                    wds.to_tuple("image", "text"),
                    wds.batched(args.batch_size, partial=not is_train),
                ])
        else:
            pipeline.extend([
                    ...
                    wds.decode("pilrgb", handler=log_and_continue),
                    wds.rename(image="jpg;png;jpeg;webp;0.png", text="txt;0.txt"),
                    wds.map_dict(image=img_process_512, text=lambda text: text_process_cfg(text, args.cfg)),
                    wds.to_tuple("image", "text"),
                    wds.batched(args.batch_size, partial=not is_train),
                ])

    num_worker_batches = 0
